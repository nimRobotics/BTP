...
class My_mechanism(object):
    ...
    def __init__(self,a,b,p,q,omega):
        ...
        self.k = 0.00001              # Initial time for animation
    ...
    def velocity_params(self):
        while self.k<3.5: # to get one complete cycle
            speed = self.c_dot(self.k)
            self.k+=0.01

            """
            Uncomment the below 3 lines and comment the rest
            to collect data for a single set of params using "c_out_2.py"
            """
            # with open('oneP_best2.csv', mode='a') as label_file:
            #    label_writer = csv.writer(label_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            #    label_writer.writerow([self.k,speed])

            if self.k>0.4 and self.k<2.1:# store the ROI
        ...
        with open('data_best.csv', mode='a') as label_file:
           label_writer = csv.writer(label_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
