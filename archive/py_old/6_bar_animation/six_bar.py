...
class My_mechanism(object):
    ...
    def rod_q_position(self,t):
        ...
        d = (py-ay)/(ax-px)
        D = ( (d*(px-c)+py)**2 - (1+d**2)*(py**2 + (px-c)**2 - self.p**2)   )**0.5
        y0 = (d*(px-c)+py+D)/(1+d**2)
        y1 = (d*(px-c)+py-D)/(1+d**2)
        x0 = c+d*y0
        x1 = c+d*y1
        set1=(x0,y0)
        set2=(x1,y1)
    ...
    def c_dot(self,t):
        ...
        c_dot = abs(c_x-self.pos_old)/0.05
        self.pos_old = c_x
        return c_dot
    ...
    def animation_m_plus(self):
        ...
        def animate(i):
            ...
            ax2.plot([0,300],[0,0],linewidth=1,color='black')
            ax2.set_xlim(0,300)
            ax2.set_ylim(0,100)
